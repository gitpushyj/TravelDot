...
3. 모달 안에 9:16 카드 미리보기가 표시된다. 하단에 `[닫기]` `[공유]` 두 버튼.
4. `[공유]` 탭 → 카드를 PNG(1080×1920)로 캡처 → `expo-sharing`이 OS 공유 시트를 호출.
5. 사용자가 인스타 스토리/쓰레드/X/저장 등을 선택. 모달은 그대로 유지(연속 공유 가능). `[닫기]`로 종료.
...
- 사용 중: `@shopify/react-native-skia`(지도용), `expo-sharing`은 신규 추가
...
- 결과 URI를 `Sharing.shareAsync(uri, { mimeType: "image/png", dialogTitle: t("share.dialogTitle") })`에 전달.
...
src/features/share/
  ...
  ShareMapModal.tsx         # 미리보기 모달 + 캡처 + 공유 호출
  yearLabel.ts              # yearMode + availableYears → 라벨 문자열 (순수 함수)
  shareMapImage.ts          # captureRef + Sharing.shareAsync 래퍼
...
4. `shareMapImage.ts` (captureRef + Sharing 래퍼)
5. `ShareMapModal.tsx` (미리보기 + 공유/닫기 + 에러 폴백)
