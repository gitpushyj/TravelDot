...
//
// pull로 들어온 row는 정의상 서버와 동기 상태이므로 last_synced_updated_at = updated_at로
// 마킹해 두어 다음 push 사이클에서 dirty로 잡히지 않도록 한다.
async function mergeRemoteRowsLocally(rows: RemoteTripRow[]): Promise<void> {
  ...
  await db.withTransactionAsync(async () => {
    for (const r of rows) {
      ...
      await db.runAsync(
        `INSERT INTO trips
           (id, country_code, start_date, end_date, body,
            created_at, updated_at, deleted_at, last_synced_updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(id) DO UPDATE SET
           ...
           last_synced_updated_at = excluded.updated_at
