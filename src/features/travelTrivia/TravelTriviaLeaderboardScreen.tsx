...
export function TravelTriviaLeaderboardScreen({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetchTriviaLeaderboard(LIMIT),
      userId ? fetchMyTriviaRank() : Promise.resolve(null),
    ]).then(([list, me]) => {
      if (cancelled) return;
      setEntries(list);
      setMyRank(me);
...
function LeaderboardRow({ entry, isMe }: { entry: LeaderboardEntry; isMe: boolean }) {
  ...
  const name = entry.nickname ?? t("trivia.anonymousTraveler");
